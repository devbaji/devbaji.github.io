import{I as y,j as L,J as P,_ as D,f as N,p as E,u as R,o as p,c as d,K as U,$ as x,a as f,D as b,X as k,F as O,l as I,m as S,O as z,t as v,a0 as M}from"./app.b3493db0.js";const te={},H=[{path:"/",title:"",pathLocale:"/",contents:[{header:"Vue 3 Google Login - Complete Documentation",content:"Using Nuxt? The recommended way to integrate this plugin in a Nuxt app is via the dedicated wrapper module \u2192 nuxt-vue3-google-login"},{header:"Overview",slug:"overview",content:`vue3-google-login is a lightweight, production-ready Vue 3 plugin for implementing Google Sign In, Google authentication, and OAuth2 login flows in your Vue 3 applications. This package provides seamless integration with Google Identity Services and the Google 3P Authorization JavaScript Library, making it easy to add Google login functionality to your Vue 3 projects.
Perfect for Vue 3 developers who need a reliable solution for Google OAuth2 authentication, Google signin buttons, and Google Identity Services integration. Full TypeScript support included.`},{header:"Key Features",slug:"key-features",content:`Login with Google button - Pre-styled Google sign-in button component
...
Lightweight & Fast - Minimal bundle size, optimized performance`},{header:"Prerequisites",slug:"prerequisites",content:`Before using vue3-google-login in your Vue 3 project, ensure you have: Vue 3.0.3 or above - This plugin requires Vue 3.0.3 or higher
Google API Client ID - To enable Google Sign In on your website, you need to set up your Google API client ID. Follow these steps to get your client ID
Local Development Setup - For testing in your local environment, add http://localhost and http://localhost:<port_number> to the Authorized JavaScript origins in your Google Cloud Console`},{header:"Getting Started - Vue 3 Google Login Setup",slug:"getting-started-vue-3-google-login-setup",content:""},{header:"Installation",slug:"installation",content:`Install the vue3-google-login package using your preferred package manager:
NPM:
...
<script src="https://cdn.jsdelivr.net/npm/vue3-google-login@2.0.36/dist/index.umd.min.js"><\/script>`},{header:"Initialize the Plugin",slug:"initialize-the-plugin",content:`Initialize the vue3-google-login plugin in your main.js or main.ts file. This will register the GoogleLogin component globally and configure Google OAuth2 authentication for your Vue 3 application:
...
}) app.mount('#app') \u{1F4A1} Tip: If you don't want to initialize and register the GoogleLogin component globally, you can directly import it from vue3-google-login package and use the client-id prop. Some functions also accept a clientId option to avoid initializing the plugin. See the Options section for more details.`},{header:"Using the GoogleLogin Component",slug:"using-the-googlelogin-component",content:`Once the plugin is installed and initialized, you can use the GoogleLogin component anywhere in your Vue 3 application. This component renders a Google sign-in button that opens a popup for Google OAuth2 authentication:
<script setup>
...
Here is an image showing the Google sign-in button rendered by Google Identity Services:`},{header:"One Tap prompt",slug:"one-tap-prompt",content:`For this feature set the prop prompt to true, this will open a prompt with the list of logged in Google accounts of the user, now user can just tap on his prefered account to easily login to your application
<script setup>
...
</template> \u2139\uFE0F If the user closes the One Tap prompt manually, the One Tap prompt will be suppressed, see here for more info Here is an image showing the Google One Tap prompt in action:`},{header:"Use of googleLogout function",slug:"use-of-googlelogout-function",content:`While using One-tap feature, a dead-loop UX issue may occur. To resolve this issue, in your logout function run googleLogout funtion
...
<\/script> <template> <div>One-Tap prompt will be shown once this component is mounted</div>
</template> Here is an image showing how the Google One Tap prompt automatically detects the logged-in Google account and logs in without user interaction:`},{header:"Get User Data",slug:"get-user-data",content:`In the triggered callback after login you will get a JWT credential string which can be decoded using decodeCredential function to retrive users basic data
<script setup>
...
Here is an image showing how a custom button opens the Google OAuth2 login popup:`},{header:"Disabling a custom button until SDK is ready",slug:"disabling-a-custom-button-until-sdk-is-ready",content:`When building a fully custom button it's a good practice to disable it until the Google SDK is fully loaded. You can use the useGoogleSdk composable which exposes a reactive isLoaded boolean for this purpose.
<script setup>
import { useGoogleSdk, googleAuthCodeLogin } from "vue3-google-login" const { isLoaded } = useGoogleSdk() const login = async () => { const response = await googleAuthCodeLogin() console.log("Handle the response", response)
}
<\/script> <template> <button :disabled="!isLoaded" @click="login"> Sign in with Google </button>
</template> \u{1F4A1} isLoaded is a Vue computed ref that becomes true once the Google Identity Services script is fully loaded and ready. Until then, the button stays disabled, preventing users from clicking before the SDK initializes.`},{header:"Using Google SDK",slug:"using-google-sdk",content:`If you want to directly use the API provided by Google Identity Services JavaScript SDK without even initializing the plugin, you can use googleSdkLoaded wrapper function to do that. This will run an action in which you can use the API directly, and under the hoods it will make sure that this action is performed only after the SDK library is fully loaded.
...
</template>`},{header:"Nuxt 3 / Nuxt 4",slug:"nuxt-3-nuxt-4",content:""},{header:"Using the Nuxt module (recommended)",slug:"using-the-nuxt-module-recommended",content:`The easiest way to use vue3-google-login in a Nuxt app is via nuxt-vue3-google-login \u2014 compatible with both Nuxt 3 and Nuxt 4. It handles SSR safety, auto-imports components and utility functions, and requires zero boilerplate.
npx nuxi@latest module add nuxt-vue3-google-login
Then add your Client ID to nuxt.config.ts:
export default defineNuxtConfig({ modules: ['nuxt-vue3-google-login'], googleLogin: { clientId: 'YOUR_GOOGLE_CLIENT_ID', },
})
All components and utility functions are auto-imported \u2014 no import statements needed:
<template> <GoogleLogin :callback="onSuccess" />
</template> <script setup>
function onSuccess(response) { const user = decodeCredential(response.credential)
}
<\/script>`},{header:"Manual setup (without the Nuxt module)",slug:"manual-setup-without-the-nuxt-module",content:`If you prefer to set it up manually, create a file named vue3-google-login.client.ts inside the plugins directory. This will register the GoogleLogin component globally. \u2757 Make sure to use the .client suffix in the file name to load the plugin only on the client side. // plugins/vue3-google-login.client.ts
...
Description clientId
String
Google API client ID`}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]}],B="update-vuepress-plugin-full-text-search2-search-index";var G=y(H),V=L(()=>{const e=new Map;for(const o of G.value)e.set(o.path,o);return e});te.webpackHot&&(__VUE_HMR_RUNTIME__[B]=e=>{G.value=e});function F(e){const o=y([]);let a=null;return P(e,()=>{a&&clearTimeout(a),a=setTimeout(i,100)}),o;function i(){const c=e.value.toLowerCase().trim();if(!c){o.value=[];return}const l=new Map,u=new Set;for(const n of G.value)for(const s of j(n,c)){u.add(s.parentPageTitle);let r=l.get(s.parentPageTitle);r||(r=[],l.set(s.parentPageTitle,r)),r.push(s)}const t=[...u].sort((n,s)=>{const r=l.get(n);return l.get(s).length-r.length});o.value=[...l].flatMap(([,n])=>n).sort((n,s)=>n.parentPagePriority-s.parentPagePriority||t.indexOf(n.parentPageTitle)-t.indexOf(s.parentPageTitle)||n.priority-s.priority)}}function*j(e,o){const a=T(e.title,o);if(a){yield{path:e.path,parentPageTitle:w(e),title:e.title,display:a,page:e,content:null,parentPagePriority:1,priority:1};return}for(const i of e.contents){const c=T(i.header,o);if(c){yield{path:e.path+(i.slug?`#${i.slug}`:""),parentPageTitle:w(e),title:e.title,display:c,page:e,content:null,parentPagePriority:10,priority:2};continue}const l=T(i.content,o);l&&(yield{path:e.path+(i.slug?`#${i.slug}`:""),parentPageTitle:w(e),title:e.title,display:[{type:"header",str:`${i.header}
`},...l],page:e,content:null,parentPagePriority:10,priority:10})}}function w(e){const o=e.path.split("/");let a="/";return o[1]&&(a=`/${o[1]}/`),(V.value.get(a)||e).title}function T(e,o){const a=[];let i=0;const c=e.toLowerCase().replace(/\s/gu," ");let l=0,u=c.indexOf(o,l);if(u<0)return null;for(;u>=0;){const n=u+o.length;if(t(e.slice(l,u),"normal"),t(e.slice(u,n),"highlight"),l=n,u=c.indexOf(o,l),i>100)break}return t(e.slice(l),"normal"),a.filter(n=>n.str);function t(n,s){let r=n;s==="normal"&&r.length>100&&i===0&&(r=`\u2026 ${r.slice(-10)}`);let h=!1;if(i+r.length>100){if(a.some(m=>m.type==="ellipsis"))return;r=r.slice(0,Math.max(100-i,1)),h=!0}a.push({type:s,str:r}),i+=r.length,h&&(a.push({type:"ellipsis",str:" \u2026"}),i+=2)}}const Y={},J=N({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>Y}},setup(e){const{locales:o}=E(e),a=y(""),i=y(!1),c=y(-1),l=F(a),u=L(()=>a.value&&i.value&&l.value.length),t=z(),n=R(),s=L(()=>{var g;return(g=o.value[n.value])!=null?g:{}});function r(){if(!u.value)return;let g=c.value-1;g<0&&(g=l.value.length-1),m(g)}function h(){if(!u.value)return;let g=c.value+1;g>=l.value.length&&(g=0),m(g)}function m(g){c.value=g}function A(){c.value=-1}function _(g){if(!u.value)return;const C=l.value[g];C&&t.push(C.path)}return{query:a,focused:i,focusIndex:c,suggestions:l,activeSuggestion:u,onUp:r,onDown:h,focus:m,unfocus:A,go:_,locale:s}}}),K={class:"search-box",role:"search"},q=["placeholder"],$=["onMousedown","onMouseenter"],W=["href"],Q={key:0,class:"parent-page-title"},X={class:"suggestion-row"},Z={class:"page-title"},ee={class:"suggestion-content"};function oe(e,o,a,i,c,l){var u;return p(),d("div",K,[U(f("input",{ref:"input","onUpdate:modelValue":o[0]||(o[0]=t=>e.query=t),"aria-label":"Search",class:b({focused:e.focused}),placeholder:(u=e.locale.placeholder)!=null?u:"Search",autocomplete:"off",spellcheck:"false",onFocus:o[1]||(o[1]=()=>e.focused=!0),onBlur:o[2]||(o[2]=()=>e.focused=!1),onKeyup:[o[3]||(o[3]=k(t=>e.go(e.focusIndex),["enter"])),o[4]||(o[4]=k((...t)=>e.onUp&&e.onUp(...t),["up"])),o[5]||(o[5]=k((...t)=>e.onDown&&e.onDown(...t),["down"]))]},null,42,q),[[x,e.query]]),e.activeSuggestion?(p(),d("ul",{key:0,class:"suggestions",onMouseleave:o[7]||(o[7]=(...t)=>e.unfocus&&e.unfocus(...t))},[(p(!0),d(O,null,I(e.suggestions,(t,n)=>(p(),d("li",{key:n,class:b(["suggestion",{focused:n===e.focusIndex}]),onMousedown:s=>e.go(n),onMouseenter:s=>e.focus(n)},[f("a",{href:t.path,onClick:o[6]||(o[6]=M(()=>{},["prevent"]))},[t.parentPageTitle&&(!e.suggestions[n-1]||e.suggestions[n-1].parentPageTitle!==t.parentPageTitle)?(p(),d("div",Q,v(t.parentPageTitle),1)):S("",!0),f("div",X,[f("div",Z,v(t.title||t.path),1),f("div",ee,[(p(!0),d(O,null,I(t.display,(s,r)=>(p(),d("span",{key:r,class:b(s.type)},v(s.str),3))),128))])])],8,W)],42,$))),128))],32)):S("",!0)])}var ie=D(J,[["render",oe],["__scopeId","data-v-5cdeda06"],["__file","SearchBox.vue"]]);export{ie as default};
